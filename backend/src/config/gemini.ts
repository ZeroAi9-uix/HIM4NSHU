...
export async function generateJoke(
  category: 'dark' | 'funny' | 'lame' | 'romance' | 'roast',
  roastTopic?: string
): Promise<string> {
  if (!genAI) {
...
export async function generateQuizQuestion(): Promise<{ setup: string; punchline: string }> {
  if (!genAI) {
...
export async function chatWithAI(
  ...
  try {
    ...
    // If no valid history, use generateContent directly
    if (!history.length) {
      const result = await model.generateContent(lastMsg.parts);
